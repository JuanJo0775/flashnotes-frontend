...
>
> Las seis tareas están hechas y verificadas: 1444 tests del front y 148 del
> back, lint, tipos y build en verde. **Se desvió del plan en tres sitios**, y
> vale la pena leerlos antes de escribir la etapa 2:
>
> **1 · Las variantes no son un comando.** El plan añadía un `//quien` con
> `match` abierto a `COMMANDS`. Habría roto dos cosas: `allCommandsFound()`
> exige que TODOS los ocultos estén desbloqueados, así que uno que no se lista
> en ningún sitio habría dejado el arte de la terminal casi inalcanzable; y un
> patrón abierto se habría tragado los comandos desconocidos de verdad, y con
> ellos el `rememberHint()` del que cuelga el faro de `//help`.
>
> Se recogen en la **rama del «comando desconocido»** dentro de `run()`. No toca
> `COMMANDS`, así que ni el arte de la terminal ni `//help` se enteran — y
> funciona sola dentro de la v0.2, donde `//whoareu` está filtrado por
> `notInV02` y cae justo ahí. Que el ente hable desde el sitio de «no te
> entiendo» resultó ser mejor lore que el comando: está encerrado, y lo único
> que le llega es lo que el sistema descarta.
>
> **2 · `//reset` no lo dormía.** El plan no lo contemplaba. Es el mismo cabo
> que se soltó una vez con `flashnotes:helpHint`, y acá habría sido peor. Ya
> está en `resetEverything()` con su test.
>
> **3 · El test de la guía se desfasaba solo.** `tests/docs/secretos.test.ts`
> anclaba el corte en el literal `'# Los 28 secretos'`, así que el secreto
> veintinueve lo dejaba vacío. Ahora el ancla es `/^# Los \d+ secretos/m`.
>
> Lo demás salió como estaba escrito. Lo que la etapa deja fuera sigue al final
> del documento.
